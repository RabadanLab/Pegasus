...
public class Gff
{
	...
	*/
	...
	throws IOException
	{
		Log.log( "Gff.read(): Reading " + filename );
	...
	*/
	private static Feature parseLine( String s ) 
	{
		...
		String seqname= s.substring( start, end );
		
		start= end + 1;
		end= s.indexOf( '\t', start );
		String source= s.substring( start, end );
     ...
     * @throws IOException Something went wrong -- check exception detail message.
	 */
	public static void write( FeatureList features, String filename ) 
	throws IOException
	{
		Log.log( "Gff.writeFile(): Writing " + filename );
	...
	private static void writeLine( Feature f, BufferedWriter bw )
	throws IOException
	{
		String s= f.seqname() + '\t';
		s+= f.source() + '\t';
	...
	throws Exception
	{
		FeatureList list= Gff.read( args[0] );
		
		Gff.write( list, args[1] );
	}
